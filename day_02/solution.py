...
def solve_puzzle_1(input_data: list) -> int:
    ...
    for item in input_data:
        ...
        for i in range(int(min_val), int(max_val) + 1):
            ...
            first, second = str(i)[:length // 2], str(i)[-length // 2:]
...
def solve_puzzle_2(input_data: list) -> int:
    ...
    Part Two
    ========
    The clerk quickly discovers that there are still invalid IDs in the ranges in your list. Maybe the young
    Elf was doing other silly patterns as well?

    Now, an ID is invalid if it is made only of some sequence of digits repeated at least twice.
    So,
        12341234 (1234 two times),
        123123123 (123 three times),
        1212121212 (12 five times), and
    ...
    for item in input_data:
        ...
        for number in range(int(min_val), int(max_val) + 1):
            length = len(str(number))
            number = str(number)

            for j in range(length):
                number_to_check = number[:j]
                occurrences = number.count(number_to_check)

                if occurrences * len(number_to_check) == length:
                    print(
                        f"Invalid ID: {number} - ",
                        f"number checked: {number_to_check} - ",
                        f"number of occurrences: {occurrences}"
                    )
                    result += int(number)
                    break
...
def main(puzzle: int, input_file: str):
    ...
    :return:
    """
    input_data = get_input_file(filename=input_file)
    input_data = [item.split("-") for item in input_data.split(",")]
